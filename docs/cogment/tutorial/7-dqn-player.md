...
In the rest of this tutorial we will use [Tensorflow and its Keras API](https://www.tensorflow.org) for the model itself, as well as [numpy](https://numpy.org) for datastructures. Let's add these to `dqn_agent/requirements.txt` and import them at the top of `dqn_agent/main.py`.
