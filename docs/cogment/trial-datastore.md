...
Cogment Trial Datastore implements the [datalog API](../concepts/core-concepts.md#additional-components). It is designed to store the trajectory samples generated by the trials for online and offline usage.
...
The TCP port where to serve the [The Data/Log API](../reference/cogment-low-level-api-guide/grpc.md#datalog-api) and the [Trial Datastore API](../reference/cogment-low-level-api-guide/grpc.md#trial-datastore-api). The former is where the orchestrator connects to output the data generated by trials, the latter is the API that can be used to retrieve online or offline stored data.
