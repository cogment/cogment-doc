...
The Directory works in tandem with the Cogment [endpoints](../../parameters.md#cogment-endpoints), in particular the ones with a [discover host](../../parameters.md#discover-host) (referred as **discovery endpoints**).
...
But it can significantly slow down directory inquiries, especially when a health check fails.
...
Each entry is normally checked for connectivity on a regular basis (every 60 seconds), but can also be checked when an entry matches an inquiry (see [check_on_inquire](#check_on_inquire)).
...
If a service fails the health check multiple times, it will be removed from the directory.
But after the first failure, the service will not be reported on inquiries until and unless it succeeds a health check.
...
1. For each service that match the inquiry, the last read "overall_load" `Status` (see below) is used. If [check_on_inquire](#check_on_inquire) is enabled, then the service's current "overall_load" is read at this point.
...
-   **properties**: This is a free form mapping of name and value strings. But Cogment may require properties of specific names for its operation, in particular for endpoint [discovery](../../parameters.md#cogment-endpoints). These special property names are prefixed with a double underscore (\_\_), e.g. `__implementation`. Also, for proper use in Cogment, property names and values should be restricted to a limited character set (see [endpoint query](../../parameters.md#discovery-query)).
